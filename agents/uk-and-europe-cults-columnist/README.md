...
- European Google News editions, per-locale cult keyword rules, per-publisher `site:` locale rules, and per-language cluster-expansion stopword lists (`base` plus `en`, `de`, `fr`, …) live in `data/google-news-europe-locales.json`, `data/google-news-locale-cult-keywords.json`, `data/publisher-host-config.json`, and `data/cluster-token-stopwords.json` (editable without changing TypeScript). Use `npm run probe:publisher-langs` to fetch homepages and record HTML/text language hints for tightening `googleNewsLocaleIds`. Headline language uses `story.sourceLanguage` when set, otherwise `tinyld` on the title, with fallback to English.
