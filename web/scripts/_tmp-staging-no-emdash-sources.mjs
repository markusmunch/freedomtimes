/**
 * One-off: staging MCP – Katie Simpson sources (no pipes, no em dashes; link on title; URL plain).
 * Plus strip U+2014 from Inès Chatin post body on staging (replace with " - ").
 */
...
const SLUG_INES = "ines-chatin-liberation-investigation-france-context";
...
function resolveToken() {
	...
	const key = STAGING.replace(/\/$/, "");
	const t = auth[key]?.accessToken;
	if (!t) throw new Error(`No token for ${key}`);
	return t;
}
...
function stripEmDashInBlocks(blocks) {
...
async function main() {
	...
	// --- Katie ---
	const katieBefore = await emdashMcpToolsCall(STAGING, tok, "content_get", {
		collection: "posts",
		id: SLUG_KATIE,
	});
	writeFileSync(
		join(backupDir, "backup-katie-pre-sources-edit.json"),
		JSON.stringify(katieBefore, null, 2),
		"utf8",
	);

	const contentK = katieBefore.item.data.content;
	const idx = contentK.findIndex(
		(b) =>
			b.style === "h2" &&
			b.children?.[0]?.text === "Sources and Further Reading",
	);
	if (idx < 0) throw new Error("Sources h2 not found on Katie post");
	const head = contentK.slice(0, idx);
	const merged = [...head, ...katieSourceTail()];

	await emdashMcpToolsCall(STAGING, tok, "content_update", {
		collection: "posts",
		id: SLUG_KATIE,
		data: { content: merged },
		_rev: katieBefore._rev,
	});

	// --- Inès: strip em dash in PT spans ---
	const inesBefore = await emdashMcpToolsCall(STAGING, tok, "content_get", {
		collection: "posts",
		id: SLUG_INES,
	});
	writeFileSync(
		join(backupDir, "backup-ines-pre-emdash-strip.json"),
		JSON.stringify(inesBefore, null, 2),
		"utf8",
	);

	const mergedInes = stripEmDashInBlocks(inesBefore.item.data.content);
	await emdashMcpToolsCall(STAGING, tok, "content_update", {
		collection: "posts",
		id: SLUG_INES,
		data: { content: mergedInes },
		_rev: inesBefore._rev,
	});

	console.log("OK: Katie sources rewritten; Inès em dashes replaced with ' - '.");
}
